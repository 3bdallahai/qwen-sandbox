...
class Qwen_model():
    ...
    def save_chat_history(self,assitant_response):
        self.prompt.append({"role":"assistant","content":assitant_response})
    ...
    def run_chatbot(self):
        while True:
            ...
            self.save_chat_history(response)    
            response = self.run_query(query)
            
            print(response)
        
    def run_from_app(self,query):
        if query == "exit":
            return 
        elif query == "clear":
            self.delete_chat_history()
